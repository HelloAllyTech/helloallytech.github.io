...
- **Scope of the sweep**, verified by grep across all seven repos and the workspace root: the `stacks` skill (`.claude/skills/stacks/SKILL.md`, eight byte-identical copies — canonical edited once and propagated, the same discipline the bridge uses); the Stacks paragraph in `CLAUDE.md`/`AGENTS.md` in `ally-ai`, `ally-web` and `infra`, which repeated the same wrong hit shape; and four bullets in the 2026-08-10 entry here. Remaining matches elsewhere are unrelated — Ansible playbooks, `Bookmark` icons, the knowledge-base epub extractor.
