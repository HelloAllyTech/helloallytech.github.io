...
> [!NOTE]
> For product conventions behind these surfaces — cross-surface consistency with the web apps, achievement/leaderboard rules, and the counsellor-in-the-field persona — search Stacks before you plan ([Planning with the Stacks MCP](../contributing/planning-with-stacks.md)). The former in-wiki pages ([UI & Interaction](../product/ui.md), [Gamification](../product/gamification.md), [User Personas](../product/user-personas.md)) are **deprecated** as of 2026-08-07 and kept for history.
...
Navigation is driven by `RootNavigator.tsx`, which switches the stack on `isAuthenticated`. The authenticated stack mounts a `DrawerNavigator` (which contains the tab navigator) plus stack screens for listening, call summary, simulations, courses, reviews, and the complete-profile gate.
...
**Courses (Track 2.0).** The Learn tab strip (`SimulationsScreen`) renders Courses, Cases, Simulations, Learning Pathways in that order, and drops any tab whose query returns zero items rather than showing an empty grid — so a tenant with no cases has no Cases tab. Courses run off `coursesApi.ts` against ally-be's `v1/learn/tracks` learner API: track list, track detail, next-item resolution, enrolment, per-item start, and the completion calls for each item type. A course opens in `CourseOverviewScreen` (sections, per-item status, progress ring) and is taken in `CourseItemPlayerScreen`, a single stack screen mounted with gestures disabled that swaps the inner player on item type rather than pushing a screen per item. Current-item state and prev/next gating live in `useTrackPlayerNavigation`; advancing is computed client-side from `TrackItemStatus.COMPLETED` or a same-session completed set, as there is no server `canAdvance`. Item players cover article, video, journal, roleplay, case, quiz (all six question types, in `players/quiz/widgets/`), and game. An article may carry inline single-select questions: `utils/articleSegments.ts` splits the body on the `<div data-ally-question="<id>">` placeholders ally-be anchors them with and an `ArticleQuestionCard` renders in each gap, so the question is met where the author placed it. A plain string split, not a DOM walk — React Native has no `DOMParser`, and `RichText` tolerates unclosed tags, so a placeholder that ends up wrapped degrades to a split mid-element rather than becoming unreachable. The answer is final (the server refuses a second one), which is why selecting an option does not submit it — the learner picks, then confirms. Those questions are the article's completion rule when present: the server completes the item on the last answer and refuses mark-as-read while any are outstanding, and the player blocks and relabels the footer to match. It counts only the questions it actually rendered, so a translated body that lost a placeholder drops that question instead of holding the footer shut. A type this binary has no player for renders `UnsupportedItemCard` rather than nothing — courses are authored on the web, so the builder can be a component type ahead of a shipped app, and the previous `default: return null` left a blank body under a live top bar with Next disabled. The card does not force Next open: advancing still keys off item status, so a game passes through while an ungraded exercise correctly holds. Roleplay and case items hand off to the existing simulation/case flow and detect return via `utils/activeTrackContext.ts` (AsyncStorage), since those completions are reported out-of-band by the simulation subsystem. `reportVideoProgress` patches the cached track detail optimistically and only invalidates on completion; `saveJournalDraft` invalidates nothing, being a debounced autosave. `recordGameResult` likewise invalidates nothing — a game component is already complete by the time a score arrives, so the call only records a personal best. `submitArticleQuestionAnswer` invalidates only when its response says the answer completed the article, since answering mid-article moves nothing the outline shows and the call fires per question. Game items never gate progression at all: ally-be completes them on open and returns the completion in the start payload, which `GamePlayer` reports on mount. The game itself is a self-contained HTML bundle rendered in a `react-native-webview`, inlined at `players/games/` from the copy ally-web serves, and it talks to the app only over postMessage. Known v1 gaps: ordering questions reorder by arrow button only, and YouTube/Vimeo/Loom sources fall back to a thumbnail plus open-externally card instead of native SDK playback. An item the author has opened for discussion (`hasDiscussion`) shows a `DiscussionThread` panel under the player, backed by `discussionApi.ts` against ally-be's `course-discussion` routes: threaded posts three deep, edit-own within 15 minutes, delete-own, and lock/moderation states driven entirely by the server's per-post `can*` flags. It is a fixed-height sibling of the player rather than part of its scroll, because every player owns a `flex:1` ScrollView and the article's read detection depends on it. A `COURSE_DISCUSSION_REPLY` row in the in-app Notifications feed pushes `CourseItemPlayerScreen` (pushed, since the player reads its item once on mount) with `highlightPostId`.
