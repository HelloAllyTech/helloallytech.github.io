...
## How to work here
- **Whenever a product judgement comes up — not only before planning** — call the stacks MCP's `search_chunks` tool with 2–3 queries on the topic (not the ticket title), and incorporate relevant returned guidance, citing chunk titles. That means before writing an implementation plan, *and* mid-implementation at each point you would otherwise invent the answer: an empty, loading, edge or failure state; a user-facing label, button or error message; what a page shows and what it omits; a threshold, limit or cadence. The `stacks` server is declared in this repo's committed `.mcp.json` and reads `STACKS_API_KEY` from the environment (never commit a key); the `stacks` skill in `.claude/skills/` carries the query technique, and a committed `UserPromptSubmit` hook injects a small search when a prompt looks product-shaped — a floor, not the rule. The rule, setup and citation format live at `wiki/contributing/planning-with-stacks.md`. Trivial mechanical changes are exempt.
