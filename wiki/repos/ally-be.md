...
- `whatsapp/` — the channel. Provider-agnostic behind a `WHATSAPP_PROVIDER` token (Meta Cloud API is the only implementation); webhook → dedupe → rate limit → consent → keyword templates → **identity** → retrieval → send, plus the admin read side (conversation log, unanswered-question queue, usage analytics). Open to anyone with the number, so the rate limit, consent gate and disclaimer are load-bearing rather than optional.
  ...
  Because the corpus is targeted per organisation, answering a question needs to know which organisation is asking, and a phone number is the only identifier WhatsApp supplies. `wa_contacts.user_id`/`tenant_id` are resolved by matching the sender against `users.phone` — **all-or-nothing**: exactly one active user must match, since picking one of two candidates would answer a worker out of a stranger's organisation. Resolution re-runs on every message, so a number added to a profile later starts working on the next question, and an admin-made link is never overwritten by the automatic match. An unresolved contact is told its number is not recognised (`handled_by = unidentified`, counted apart from `declined` because the two have opposite fixes) and is **never** answered from the global corpus — that would mean the same question got a different answer depending on whether we happened to recognise the sender.
