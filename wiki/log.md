...
## [2026-08-19] fix | Routing-index fan-out could not merge into the protected repos; it now pushes direct, and 57 stale PRs were cleared.
- **The symptom.** `chore: sync wiki routing index` had accumulated 19 open PRs each in `ally-be`, `ally-ai` and `ally-web` — one per wiki push since 2026-08-07, every one a full regeneration of the same `WIKI-ROUTING.md` and therefore superseding all the older ones. `ally-ai-learn` had none, which is what located the cause: it is the only one of the four fan-out targets with no branch protection.
- **The cause.** `routing-index.yml` opened a PR and armed auto-merge on it. The three protected repos require one approving review, and **a GitHub App cannot approve its own pull request**, so auto-merge stayed armed and never fired. Both `gh pr merge` attempts were redirected to `/dev/null` and the fallback was a `::warning::`, which does not fail the job — so twelve days of green runs hid a growing queue.
- **A dead end worth recording.** Classic branch protection has no setting that waives a required review for a chosen actor. `bypass_pull_request_allowances` reads like one but grants "may push without opening a PR at all": adding `ally-docs-bot` to it left `REVIEW_REQUIRED` in place on a PR whose four checks were green and whose branch was current. Repository rulesets do have a bypass covering required reviews, but moving the review rule out of classic protection would change how `master` is guarded for everyone, not just the bot.
- **The fix.** The step now commits `WIKI-ROUTING.md` straight to the default branch and pushes, rebasing up to three times if the tip moved. That is what the bypass actually authorises, and with no pull request there is no queue to strand. It fails the job with the push output on the way out rather than falling back to a PR — a branch the bot cannot push means a cleared bypass or a protection change, and both want a human rather than another PR nobody merges.
- **The cleanup.** Merged the newest snapshot in each repo and closed the other 54 as superseded, deleting their branches.
- Filed the underlying lesson in `memory.md`: an App cannot approve its own PR, `bypass_pull_request_allowances` is not a review waiver, a bot write path that needs a protection exemption owns that exemption, and auto-merge will not update a stale branch for you.
