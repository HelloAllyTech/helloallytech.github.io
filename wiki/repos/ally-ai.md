...
- `/knowledge-chunks` — write and read side of the `KnowledgeChunk` collection for the WhatsApp Q&A bot: `bulk-upsert` (per-object success/failure so ally-be can retry only what failed), `search`, `document/{id}` (delete by document), `document/{id}/audience` (retarget an indexed document at organisations, in place), `ids` (paged, for reconciliation), and `{chunk_id}`.
...
- **Retrieval is scoped to an organisation** by `is_global` and `tenant_ids` (migration `005`), mirrored from ally-be's `kb_documents.is_global` and `kb_document_tenants`. This replaced the collection's original rule that a per-tenant corpus should be a NEW collection rather than a filter on the shared one — one collection per tenant does not survive a single document shared by three of them. The reasoning behind that rule ("retrieval that forgets a filter leaks, and an un-set filter is the easiest thing in the world to forget") is answered structurally instead, and the three mechanics below are the answer.

Three things make the audience filter hard to get wrong, and they are worth knowing before touching either service:

- **The filter is applied INSIDE the vector query**, as a disjunction (`is_global == true OR tenant_ids contains mine`), never to its results. A post-filter over a fixed top-k starves recall rather than restricting it: an organisation with five documents among ten thousand global chunks would retrieve none of them, and the bot would report that as "my reference material does not cover that".
- **The audience cannot be omitted.** `KnowledgeChunkService.search` and the answering agent both take a REQUIRED audience argument; searching the whole corpus means naming `ChunkAudience.unrestricted()` (admin tooling only), an audience that can match nothing returns nothing rather than everything, and a chunk indexed with no audience reaches nobody rather than everybody. `POST /knowledge-agent/answer` refuses a request with no `audience` with a 422 — there is no default, because both plausible defaults are silently wrong.
...
Migration `005` backfills existing objects to `is_global = true`. That is not a guess about intent: every object already in the collection was indexed while the corpus was global to everyone, and a property added later reads back as null, which satisfies neither half of the filter — without the backfill, shipping the filter would take the whole live corpus out of retrieval at once. ally-be's matching migration makes the same statement about its rows, so the two stores agree without either reading the other.

Which organisation is asking is ally-be's decision, not this service's: it matches the WhatsApp sender's number against `users.phone` and refuses to answer a contact it cannot resolve. See ally-be's `DATA_SCHEMA.md` §3.11.
