...
- `roleplay-studio/` — Roleplay Studio v2: versioned Scenario Spec + copilot authoring, rehearsal lifecycle (simulated trainees + judge, webhook-driven), evidence-rich critique with persisted proposals, and the **auto-improve loop** (rehearse → critique → apply → re-rehearse on a scratch version lineage until targets are met). The flow is **chat-first**: the copilot offers test cases, starts the loop itself, narrates progress into the persisted chat, auto-applies the winner on success, and surfaces test-live/publish from the conversation. See the repo's `ROLEPLAY_STUDIO_V2.md` for the cross-repo reference.
