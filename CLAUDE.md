...
## How to work here
- **Whenever a product judgement comes up — not only before planning** — run `/stacks:planning_context` with a description of the task, and incorporate relevant returned guidance, citing chunk titles. That means before writing an implementation plan, *and* mid-implementation at each point you would otherwise invent the answer: an empty, loading, edge or failure state; a user-facing label, button or error message; what a page shows and what it omits; a threshold, limit or cadence. **There is no search tool** — `planning_context` is an MCP prompt, so only a human can invoke it: a session can neither query the library itself nor list what it holds, and must never claim Stacks does or doesn't cover something. When guidance would help and no Stacks context block is in the conversation, run the prompt rather than guessing; use the stacks MCP's `get_chunks` tool for the full passage behind any chunk id it returned. The `stacks` server is declared in this repo's committed `.mcp.json` and reads `STACKS_API_KEY` from the environment (never commit a key); the `stacks` skill in `.claude/skills/` carries the retrieval technique, and a committed `UserPromptSubmit` hook pulls a small context block when a prompt looks product-shaped — a floor, not the rule. The upstream embedder is capped at 3 requests/minute across everyone, so an empty result is often just the ceiling. The rule, setup and citation format live at `wiki/contributing/planning-with-stacks.md`. Trivial mechanical changes are exempt.
