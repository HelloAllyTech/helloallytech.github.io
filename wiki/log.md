...
## [2026-09-15] fix | Stacks attribution vocabulary retired platform-wide; the 2026-08-10 entry amended in place.

> **This entry amends earlier entries.** The log is append-only by convention, and the 2026-08-10 entry was edited rather than superseded — recorded here so the exception is visible rather than silent. Nothing about what happened on 2026-08-10 changed; only the vocabulary describing it, plus one factual correction noted below.

- **Why the exception was taken.** The wording is not only in prose an agent skims — it sits in the `stacks` skill and repo instructions that every session loads, so a stale term keeps re-teaching itself. Leaving it correct going forward but wrong in history would have meant the published log contradicted both the page and the tooling.
- **The 2026-08-10 entry carried the same factual error the page did.** It described a search hit as `title, <attribution>, section, framing sentence, id`. That field does not exist. Corrected there to the real shape (`title · §section · tags · score · id`, plus one framing sentence) — so the amendment fixed an inaccuracy, not just a word.
- **Scope of the sweep**, verified by grep across all seven repos and the workspace root: the `stacks` skill (`.claude/skills/stacks/SKILL.md`, eight byte-identical copies — canonical edited once and propagated, the same discipline the bridge uses); the Stacks paragraph in `CLAUDE.md`/`AGENTS.md` in `ally-ai`, `ally-web` and `infra`, which repeated the same wrong hit shape; and four bullets in the 2026-08-10 entry here. Remaining matches elsewhere are unrelated to Stacks — Ansible terminology, admin icon names, and the knowledge-base document extractor, which handles uploaded reference material and legitimately uses the old term.
- **One trap for anyone repeating this sweep:** `git grep -E` uses POSIX ERE, where `\b` is *not* a word boundary. A word-boundary pattern such as `'\bterms?\b'` returns zero matches in repos that plainly contain the term, silently. It reported every repo clean before the pattern was corrected to a plain substring with compound words filtered out.
- **Not in reach from here:** the Stacks MCP server's own tool descriptions and server instructions still use the term, and they are authored in the server repo rather than in any Ally repo. A session reads those every time it connects, so the vocabulary is not fully retired until they are updated too.

## [2026-09-15] fix | Stacks page's setup story was a version behind; obsolete attribution wording retired.
- `contributing/planning-with-stacks.md`: the **Configuration** section still described the pre-bridge world — "get a key from the platform team", `export STACKS_API_KEY` in `~/.zshrc`, and `.mcp.json` reading `${STACKS_API_KEY}` at connect time. The auto-provisioning bridge landed 2026-08-10 (`ally-be` `2e0b623d`); this page was edited on 2026-08-13 and the setup section was never reconciled with it, so for five weeks the wiki told new engineers to go and ask for a credential that no longer exists.
- Replaced with what is actually true: `.mcp.json` carries no endpoint and no credential and only execs `.claude/stacks-bridge.mjs`, which resolves a key in three tiers (env → `~/.claude/.stacks-key` at 0600 → `gh auth token` exchanged at `POST /api/mcp/exchange`). Setup is `gh auth login`, once per machine.
- Recorded the failure mode that wording caused, not just the correction: because tier 2 short-circuits, **an unset `STACKS_API_KEY` is the normal state on a working machine.** The old troubleshooting list opened with `echo $STACKS_API_KEY`, which sends anyone debugging a genuine outage down a path where healthy looks broken. The new list points at the bridge's stderr, which names its own fix on every failure path, and the four messages are tabulated.
- Added the bridge's two structural properties, both of which are operational traps: it **fails loud by design** (no key means no server at all — the opposite of the hook it replaced, which failed open and silent), and its **nine copies have no sync mechanism** (canonical `clients/stacks-bridge.mjs` in the Stacks server repo, plus eight deployed), so a drifted copy reads as a server fault. Verified all eight deployed copies identical at time of writing.
- Separately, retired the page's obsolete source-attribution vocabulary throughout. This was not only a wording change: the page described a search hit as carrying an attribution field alongside the section, and the API returns no such field. Both live tool descriptions now state that results carry no attribution beyond the section, and a real `search_chunks` call returns `title · §section · tags · score · id` plus a framing sentence. Corrected to the real shape.
- Added why attribution stops at the section: a chunk is the team's own distilled principle, to be weighed on whether it fits the decision in hand — not a reference to go and look up. The four remaining occurrences were resolved as "source" (`get_chunks` summaries), "source catalogue" (the `list_documents` note), "new material" (the moving-target note) and "corpus ingestion" (the `EMBED_MAX_RPM` warning).
...
- Two safety rules are recorded because both have bitten: a filler must never contradict the reply that follows it (so generated phrases are checked in code, not only by the prompt), and clip rendering never runs on the session's own speech engine (a run of synthesis errors there closes the session — it has taken live sessions down).
- Also records the measurement trap this feature carries: a filler counts as the character's first words, so response latency is measured to it. The page notes that each turn keeps which audio was heard first alongside the unmasked time to the real reply, so rising filler coverage can never be misread as a latency win.
- Sanitized for public hosting: concepts only — no provider names tied to failures, no hostnames, no environment or region details.

## [2026-08-19] fix | Routing-index fan-out queued 57 unmergeable PRs; now force-pushes one reusable branch per repo.
- **The symptom.** `chore: sync wiki routing index` had 19 open PRs each in `ally-be`, `ally-ai` and `ally-web` — one per wiki push since 2026-08-07, each a full regeneration of the same `WIKI-ROUTING.md` and therefore superseding every older one. `ally-ai-learn` had none, which located the cause: it is the only fan-out target with no branch protection.
- **The cause.** The workflow opened a PR and armed auto-merge. The other three targets require one approving review, and **a GitHub App cannot approve its own pull request**, so auto-merge stayed armed and never fired. Both `gh pr merge` attempts were sent to `/dev/null` behind a `::warning::`, which does not fail the job — twelve days of green runs hid the queue.
- **Two dead ends, both recorded so nobody retries them.** Classic branch protection offers no per-actor review waiver. `bypass_pull_request_allowances` reads like one but grants only "may push without opening a PR": adding `ally-docs-bot` left `REVIEW_REQUIRED` in place on a PR with four green checks and a current branch. Pushing direct under that same bypass is then refused by the required status check itself — `GH006 ... Required status check "gitleaks" is expected` — because the check cannot run before the ref moves. **Required status checks gate direct pushes, not merges only.**
- **The fix is shape, not permission.** The step now force-pushes a single long-lived `chore/sync-wiki-routing` branch per target and reuses its PR, so the queue cannot grow past one and that one always holds the current index. It still attempts auto-merge — which lands unattended on `ally-ai-learn` — and emits a `::notice::` where a human is needed. It fails the job only when the regenerated index would be *lost* (branch unpushable, PR uncreatable), not when it is merely awaiting review.
- **What would make it fully unattended**, if wanted later: a repository *ruleset* bypass, which unlike classic protection can waive required reviews for an actor, or a second App whose only job is approving this PR. Both are noted in the workflow comment.
- **The cleanup.** Merged the newest snapshot in each repo and closed the other 54 as superseded, deleting their branches. The `bypass_pull_request_allowances` grant added while diagnosing buys nothing under this design and should be cleared.
- Lesson filed in `memory.md`: when each run fully supersedes the last, per-run branches are a leak; and a step whose whole job can fail must never discard the reason — printing the push output surfaced `GH006` in a single run.
...
- **The narrowing was reversed.** `search_chunks` and `list_tags` are callable again, so a session reaches the library at the moment a judgement comes up rather than waiting for a human to run `/stacks:planning_context`. The prompt stays as the human entry point. `list_documents` was not restored: a source catalogue is browsing, whereas tags feed back into a search filter.
- **Not a plain revert.** `search_chunks` now returns compact hits — title, section, tags, score, id and one framing sentence — instead of full bodies, on its own 4/10 result budget. Full bodies at the old default cost ~3.2k tokens a search, so a session searching ten times while coding would have spent ~32k tokens of its own context on prose it skims. `get_chunks` keeps depth one call away, which is what makes searching often affordable.
- **The hook is gone, from all seven repos.** `.claude/settings.json` and `.claude/hooks/stacks-search.sh` are deleted. It only ever existed because an agent could not search; keeping it would mean two paths spending the same quota on overlapping queries. Its structural limit is worth remembering: it fired on prompt text, so it could never reach the library mid-implementation, where most product decisions are made.
- **A rate-limit error now says so.** Previously a throttled search surfaced as a generic tool failure, which a session reasonably read as "the library is unavailable" before going back to inventing the answer.
- **The 3 requests/minute figure was wrong for retrieval** and has been struck from this wiki. It is `EMBED_MAX_RPM`, a pessimistic guess at the embedder's free tier applying only to corpus ingestion; the search path never passed through it. The real ceiling is unmeasured.
- **Corpus reality check.** A live search for "empty state design patterns" returned four weak matches (~0.5) all from the gamification area, none about empty states. The largest tags are `feedback-loops` (120), `instructional-design` (77), `simulation-design` (59). The corpus leans heavily toward gamification, instructional design and systems thinking, so general product and UI questions will often return confident-looking irrelevance until more material is ingested.
...
## [2026-09-03] update | Transcript lines now declare which kind of client utterance they are.
- `repos/ally-ai-learn.md`: added a paragraph to the thinking-filler section on the `utterance_kind` marker the worker now publishes with each transcript message (`reply`, `filler`, `interim`). The filler section already explained that a filler counts as the character's first words and so is what response latency is measured to; this is the same ambiguity reaching the transcript rather than the metrics, and it is resolved the same way — by recording which kind of thing was heard rather than inferring it.
- The consequence is cross-repo, which is why it belongs on a page other services read: a filler and an interim are persisted as ordinary client messages, same sender and same table as a real reply, so anything counting or judging client turns has to exclude them. Before the marker, ally-be's language judge scored every "Hmm" as a turn of roleplay — inflating the denominator under every error rate on that dashboard and inviting annotations on utterances that are meant to be contentless.
- Documented the absence rule alongside it, because it is the part that is easy to get backwards: the marker is written by the live worker only, so a line without one means *unknown*, not *reply*. Downstream reads those as replies, which is correct for a corpus recorded before the feature existed and would otherwise silently shrink.
